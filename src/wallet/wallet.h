...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zeskspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 ESK at the same time
...
enum ZerocoinSpendStatus {
    ZESK_SPEND_OKAY = 0,                            // No error
    ZESK_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZESK_WALLET_LOCKED = 2,                         // Wallet was locked
    ZESK_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZESK_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZESK_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZESK_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZESK_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZESK_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZESK_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZESK_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZESK_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZESK_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZESK_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZESK_SPENT_USED_ZESK = 14,                      // Coin has already been spend
    ZESK_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    ZESK_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void ZEskBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreateZESKOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    std::string GetUniqueWalletBackupName(bool fzeskAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzESKTracker> zeskTracker;
    ...
    void setZWallet(CzESKWallet* zwallet)
    {
        zwalletMain = zwallet;
        zeskTracker = std::unique_ptr<CzESKTracker>(new CzESKTracker(strWalletFile));
    }

    CzESKWallet* getZWallet() { return zwalletMain; }
    ...
    void setZEskAutoBackups(bool fEnabled)
    {
    ...
    /** zESK reset */
    boost::signals2::signal<void()> NotifyzESKReset;
