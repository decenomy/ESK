...
public:
    ...
    bool WriteZESKSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZESKSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZESKSeed_deprecated(uint256& seed);
    bool EraseZESKSeed();
    bool EraseZESKSeed_deprecated();

    bool WriteZESKCount(const uint32_t& nCount);
    bool ReadZESKCount(uint32_t& nCount);
