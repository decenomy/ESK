...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered eskacoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 ESK.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
static void NotifyzESKReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzESKReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzESKReset.connect(boost::bind(NotifyzESKReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzESKReset.disconnect(boost::bind(NotifyzESKReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zeskTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus, fWrongSeed);
}
