...
#include "ui_zeskcontroldialog.h"
...
std::set<std::string> ZEskControlDialog::setSelectedMints;
std::set<CMintMeta> ZEskControlDialog::setMints;

bool CZEskControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == ZEskControlDialog::COLUMN_DENOMINATION || column == ZEskControlDialog::COLUMN_VERSION || column == ZEskControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


ZEskControlDialog::ZEskControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZEskControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZEskControlDialog::~ZEskControlDialog()
{
    delete ui;
}

void ZEskControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZEskControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CZEskControlWidgetItem* itemDenom(new CZEskControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CZEskControlWidgetItem *itemMint = new CZEskControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zeskTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zeskTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zESK.";
            else if (!mint.isSeedCorrect)
                strReason = "The zESK seed used to mint this zESK is not the same as currently hold in the wallet";
            else
...
void ZEskControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZEskControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZEsk_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZEskControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZEskControlDialog::GetSelectedMints()
{
...
void ZEskControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
