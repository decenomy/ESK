// Copyright (c) 2018-2019 The ESKACOIN developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zesk/deterministicmint.h>
#include "zesktracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzESKTracker::CzESKTracker(std::string strWalletFile)
{
...
CzESKTracker::~CzESKTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzESKTracker::Init()
{
...
bool CzESKTracker::Archive(CMintMeta& meta)
{
...
bool CzESKTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzESKTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzESKTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzESKTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzESKTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzESKTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzESKTracker::GetSerialHashes()
{
...
CAmount CzESKTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzESKTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzESKTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzESKTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzESKTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzESKTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzESKTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzESKTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzESKTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzESKTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzESKTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzESKWallet* zESKWallet)
{
    bool iszESKWalletInitialized = (NULL != zESKWallet);
    CMintMeta meta;
    ...
    if (! iszESKWalletInitialized)
        zESKWallet = new CzESKWallet(strWalletFile);
    meta.isSeedCorrect = zESKWallet->CheckSeed(dMint);
    if (! iszESKWalletInitialized)
        delete zESKWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzESKTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzESKTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzESKTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzESKTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzESKTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzESKTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzESKWallet* zESKWallet = new CzESKWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            if (fExcludeV1 && dMint.GetVersion() < 2)
                continue;
            Add(dMint, false, false, zESKWallet);
        }
        delete zESKWallet;
        LogPrint("zero", "%s: added %d dzesk from DB\n", __func__, listDeterministicDB.size());
    }
