...
CzESKWallet::CzESKWallet(std::string strWalletFile)
{
...
bool CzESKWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZESKCount(nCountLastUsed);
    else if (!walletdb.ReadZESKCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzESKWallet::Lock()
{
    seedMaster = 0;
}

void CzESKWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzESKWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZESK(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzESKWallet::LoadMintPoolFromDB()
{
...
void CzESKWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzESKWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzESKWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zeskTracker->HasPubcoinHash(pMint.first)) {
...
bool CzESKWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZESK(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    pwalletMain->zeskTracker->Add(dMint, true);
...
void CzESKWallet::SeedToZESK(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzESKWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzESKWallet::GenerateDeterministicZESK(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzESKWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzESKWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzESKWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
