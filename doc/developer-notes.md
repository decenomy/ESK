...
- [Development guidelines](#development-guidelines)
    - [General ESKACOIN Core](#general-eskacoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
ESKACOIN Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play ESKs (tESK)" on the test network, if you
...
ESKACOIN Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/Eskacoin/Eskacoin-Project/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_eskacoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_eskacoin --log_level=test_suite
$ valgrind -v --leak-check=full src/eskacoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_eskacoin.coverage/index.html`.
```
...
- BitcoinMiner : Generates ESKs (if wallet is enabled).
...
However, in open source software such as ESKACOIN Core, where everyone uses
...
pay attention to for reviewers of ESKACOIN Core code.

General ESKACOIN Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ESKACOIN Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against ESKACOIN Core so that they can be integrated
...
file descriptors it will cause problems with ESKACOIN's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:Eskacoin/Eskacoin-Project.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `eskacoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `eskacoin-cli` and the GUI debug console use this table to determine how to
